...
//        Session session = entityManager.unwrap(Session.class);
//
//        session.saveOrUpdate(employee);                 //метод добавляет работника, либо изменяет его

        Employee newEmployee = entityManager.merge(employee);                     //метод добавляет работника, либо изменяет его
